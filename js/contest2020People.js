...
function createHTMLString(index) {
  ...
  container.innerHTML = 
  ` <div class="title">
        <div class="prize">${item[index].Person}</div>
        <div class="name">${item[index].Painter}</div>
      </div>
      <div class="contest-image"><img src="${item[index].url}" alt="${item[index].Person}" width="90%"></div>
  `;
}